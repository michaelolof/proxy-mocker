...
export function getUrlPath(input: string) {
...
export function JSONDecoder(input: Uint8Array[] = []): any {
    try {
...
export function matchURL(pattern: string, urlPath: string): MatchResult {
    ...
    const regexString = cleanedPattern.replace(
        /:((\w+))|{(((\w+)))}/g,
        (match, p1, p2, p3, p4) => {
